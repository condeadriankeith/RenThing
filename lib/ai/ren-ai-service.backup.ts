...
export class RenAIService {
  ...
  async generateDetailedUserPersona(userId: string) {
    try {
      ...
      const categoriesRented = user.bookings.map(booking => booking.listing.category || booking.listing.description);
      const favoriteCategories = [...new Set(categoriesRented)].filter(Boolean);
      
      const rentedPrices = user.bookings.map(booking => booking.listing.price);
      const priceRangeRented = rentedPrices.length > 0 ? {
        min: Math.min(...rentedPrices),
        max: Math.max(...rentedPrices),
        avg: rentedPrices.reduce((sum, price) => sum + price, 0) / rentedPrices.length
      } : { min: 0, max: 0, avg: 0 };

      // Extract listing patterns
      const categoriesListed = user.listings.map(listing => listing.category || listing.description);
      const listedCategories = [...new Set(categoriesListed)].filter(Boolean);
      
      const listedPrices = user.listings.map(listing => listing.price);
      const priceRangeListed = listedPrices.length > 0 ? {
        min: Math.min(...listedPrices),
        max: Math.max(...listedPrices),
        avg: listedPrices.reduce((sum, price) => sum + price, 0) / listedPrices.length
      } : { min: 0, max: 0, avg: 0 };

      // Analyze temporal patterns
      const bookingHours = user.bookings.map(booking => booking.createdAt.getHours());
      const preferredBookingHours = this.getPreferredTimeSlots(bookingHours);
      
      const bookingDays = user.bookings.map(booking => booking.createdAt.getDay());
      const preferredBookingDays = this.getPreferredDays(bookingDays);

      // Analyze seasonal patterns
      const bookingMonths = user.bookings.map(booking => booking.createdAt.getMonth());
      const seasonalPreferences = this.getSeasonalPreferences(bookingMonths);

      // Wishlist analysis
      const wishlistCategories = user.wishlist.map(item => item.listing.category || item.listing.description);
      const wishlistPriceRange = user.wishlist.length > 0 ? {
        min: Math.min(...user.wishlist.map(item => item.listing.price)),
        max: Math.max(...user.wishlist.map(item => item.listing.price)),
        avg: user.wishlist.reduce((sum, item) => sum + item.listing.price, 0) / user.wishlist.length
      } : { min: 0, max: 0, avg: 0 };

      // Review analysis
      const avgRatingGiven = user.reviews.length > 0 ? 
        user.reviews.reduce((sum, review) => sum + review.rating, 0) / user.reviews.length : 0;

      // Determine user type based on activity
      let userType = 'casual';
      if (user.bookings.length > 15 && user.listings.length > 8) {
        userType = 'power_user';
      } else if (user.bookings.length > 5 || user.listings.length > 3) {
        userType = 'active';
      }

      // Determine engagement level
      const totalInteractions = user.bookings.length + user.listings.length + user.wishlist.length + user.reviews.length;
      const engagementLevel = totalInteractions > 25 ? 'high' : totalInteractions > 12 ? 'medium' : 'low';
      ...
      const behavioralPatterns = this.identifyBehavioralPatterns(user);
   ...
   * @param hours Array of booking hours
   * @returns Object with time slot preferences
   */
  private getPreferredTimeSlots(hours: number[]): Record<string, number> {
    ...
    hours.forEach(hour => {
      if (hour >= 6 && hour <= 11) slots.morning++;
      else if (hour >= 12 && hour <= 17) slots.afternoon++;
      else if (hour >= 18 && hour <= 23) slots.evening++;
      else slots.night++;
    });
   ...
   * @returns Object with day preferences
   */
   ...
   * @returns Object with seasonal preferences
   */
  ...
  async updateExplicitUserPreferences(userId: string, preferences: {
    ...
    try {
      ...
      return {
        ...
        preferredBookingDays: updatedPreferences.preferredBookingDays ? JSON.parse(updatedPreferences.preferredBookingDays) : [],
        preferredBookingHours: updatedPreferences.preferredBookingHours ? JSON.parse(updatedPreferences.preferredBookingHours) : []
      };
    } catch (error) {

// Fixed Prisma client import issue
import { PrismaClient } from '@prisma/client';
import type { Prisma } from '@prisma/client';



import type { AIResponse } from '@/ren-ai/services/ren-ai-service-client';
...
export interface AIContext {
  ...
  userGoal?: {
    type: 'booking' | 'listing' | 'search' | 'account' | 'payment' | 'support' | 'wishlist' | 'review' | 'other';
    details: any;
...
export class RenAIService {
  ...
  // Define wizards with proper typing
  private wizards: { [key: string]: Wizard } = {
    listing: {
      steps: [
        { 
  ...
  private async processWithOpenRouter(message: string, context: AIContext): Promise<AIResponse | null> {
    ...
    // Since ChatService doesn't have sendMessage, we'll implement a placeholder
    // In a real implementation, this would call the OpenRouter API
    console.warn('OpenRouter integration not implemented');
    return null;
  }
  ...
  /**
   * Get personalized recommendations for a user (alternative implementation)
   * @param userId The user ID to get recommendations for
   * @returns Array of recommended listings
   */
  async getAlternativeRecommendations(userId: string) {
  ...
  async getPersonalizedRecommendations(userId: string) {
    try {
      // Generate detailed user persona
      const persona = await this.generateDetailedUserPersona(userId);
      if (!persona) {
        return await this.getTrendingListings();
      }
  ...
  /**
   * Get predictive navigation suggestions based on user activity (alternative implementation)
   * @param userId The user ID to get suggestions for
   * @returns Array of navigation suggestions
   */
  async getAlternativePredictiveNavigationSuggestions(userId: string) {
    try {
      // Get user's recent activity
      const userActivity = await this.getUserActivity(userId);
